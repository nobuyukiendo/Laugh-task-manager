...
const SortableItem = ({ id, children, disabled }: { id: string, children: React.ReactNode, disabled?: boolean }) => {
    ...
    return (
        <div ref={setNodeRef} style={style}>
            {/* Inject drag props into children so the handle can use them */}
            {React.cloneElement(children as React.ReactElement, { dragHandleProps: { ...attributes, ...listeners }, isDragging })}
        </div>
    );
};
    ...
    const handleDragEnd = async (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            ...
            if (oldIndex !== -1 && newIndex !== -1) {
                ...
                // Update orders in DB
                // Only update items that actually changed index to minimize DB writes?
                // Or just bulk update everything in the range?
...
`}
                                            // @ts-ignore - Props injected by SortableItem
                                            // dragHandleProps will be used by the grip button
                                        >
                                            ...
                                            <div className="flex items-center gap-2 absolute top-4 right-4">
                                                ...
                                                {!isFiltered && (
                                                    <div 
                                                        ...
                                                        {...(arguments[0] as any).dragHandleProps} 
                                                    >
